...
/**
 * Autonomous program for putting climbers in the shelter and going up the red ramp on home side
 *
...
public class T9015Auto_R_HomeClimberRamp extends T9015Hardware {
    private int v_state;
    ...
    public T9015Auto_R_HomeClimberRamp(){
    ...
    public void loop(){
        switch(v_state) {
            ...
            case 13:
                ...
                distance = 5;  //set distance to move (cm)
                power    = 0.3;  //set power
                telemetry.addData("3 - ","forward=" + distance + "p=" + power); //displays distance and power to phone screen
                if (has_driver_forward_cm(distance, power)) //when encoder has reached corresponding ticks for set distance move to the next state
                    move_to_next_state();
